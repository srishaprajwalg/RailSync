...
- **Optimization Engine (OR-Tools CP-SAT)**: A single global spatiotemporal constraint model that simultaneously schedules tasks while respecting capacity across the entire corridor.
  - Configurable horizon lengths using absolute minutes from Day 0, capable of solving a full 30-day horizon in a single run.
...
### Frontend (React/Vite)
- **Dashboard**: A React-based interface utilizing Tailwind CSS v4, featuring a 3-tab layout (Data & Setup, Control Room, Action Plan) and a robust **Time Window Selector** (Month | Week | Day) to filter the 30-day optimization results globally without triggering re-optimization.
...
- **Dual Visualization Engine**: 
  - **Technical View (CorridorTimeline)**: A custom SVG Time-Distance string chart showing exact spatiotemporal movements of passenger/freight trains and safe maintenance blocks.
  - **Simple View (SimpleGanttView)**: A simplified, clean Gantt chart that renders overlapping filtered timetable events and CP-SAT maintenance blocks without faking train gaps. Includes adaptive calendar and time ticks that automatically scale for Month, Week, or Day views based on the application's actual planning start date.
...
5. **CP-SAT Optimization**: User triggers the optimizer to generate a complete 30-day plan (`POST /api/optimize`). The CP-SAT engine calculates exact spatiotemporal train overlaps, applies compatibility rules, enforces global resource capacities, and strictly excludes any `Completed` tasks.
